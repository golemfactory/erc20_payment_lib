    ...
    pub async fn eth_transaction(
        ...
        loop {
            ...
            if let Some(idx) = idx {
                ...
                match res.await {
                    Ok(Ok(balance)) => {
                        self.mark_rpc_success(idx, "transaction".to_string());
                        return Ok(balance);
                    }
                    Ok(Err(e)) => {
                        log::warn!("Error getting balance from endpoint {}: {}", idx, e);
                        self.mark_rpc_error(idx, "transaction".to_string(), VerifyEndpointResult::RpcError(e.to_string()));
                        if loop_no > 3 {
                            return Err(e);
                        }
                    }
                    Err(e) => {
                        ...
                        self.mark_rpc_error(idx, "transaction".to_string(), VerifyEndpointResult::Unreachable);
                        if loop_no > 3 {
